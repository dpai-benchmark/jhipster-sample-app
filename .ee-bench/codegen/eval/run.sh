...
MAVEN_TEST_CMD=(./mvnw -Dtest=AuthorityResourceIT test -q)
...
_run_tests() {
  ...
  find "$PROJECT_ROOT" \( -path "*/target/surefire-reports/*.xml" -o -path "*/target/failsafe-reports/*.xml" \) -exec cp {} "$ARTIFACTS_DIR/" \; 2>/dev/null || true
